...
Contains **portable hardware abstraction interfaces** (V-Tables) that define the contract between high-level code and hardware implementations.

## Rules

- ✅ **Pure C interfaces** with function pointer tables (vtables)
- ✅ **Platform-independent** - no hardware-specific includes
- ✅ **Used by**: Infrastructure adapters and BSP implementations
- ❌ **NEVER** include `stm32*.h` or any vendor HAL headers here

## Structure

```
interfaces/
├── i_gpio.h      # GPIO interface (read, write, config)
├── i_uart.h      # UART interface (transmit, receive, DMA)
├── i_spi.h       # SPI interface (transfer, config)
├── i_i2c.h       # I2C interface (read, write, scan)
├── i_timer.h     # Timer interface (PWM, OC, counter)
├── i_rtc.h       # RTC interface (time, alarm, calendar)
├── i_exti.h      # External interrupt interface
├── i_flash.h     # Flash memory interface
├── i_dma.h       # DMA interface
└── i_power.h     # Power management interface
```
...
High-level adapters depend on these interfaces, not on concrete BSP implementations.
